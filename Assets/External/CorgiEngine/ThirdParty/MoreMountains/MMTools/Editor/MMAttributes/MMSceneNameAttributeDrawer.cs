...
{
    [CustomPropertyDrawer(typeof(MMSceneNameAttribute))]
    public class MMSceneNameAttributeDrawer : PropertyDrawer
    {
        int _sceneIndex = -1;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (GUILayout.Button($"{property.stringValue}", EditorStyles.popup))
            {
                ...
                SearchWindow.Open(new SearchWindowContext(buttonScreenPos.center, buttonScreenPos.width),/*Event.current.mousePosition)),*/ 
